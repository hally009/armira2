...
class PspController extends Controller
{
    ...
    public function index()
    {
        ...
        $items = $this->repository->getSatkerPengelolaan(jenis_pengelolaan('psp'), date('Y'));
        $temp = $this->repository->getTempPengelolaan(jenis_pengelolaan('psp'), date('Y'));

        return view('satker.psp.index', compact(['menu', 'items', 'temp']));
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        return view('satker.psp.show', compact(['menu', 'item', 'alur']));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$temp->file) {
            return redirect()->route('Satker::psp.index')->withError('Pengajuan Gagal, Dokumen Belum diupload');
        }
        ...
        try {
            ...
            $this->storeNotif([
                'role' => roles('uapb'),
                'dari' => roles('satker'),
                'content' => 'Permohonan pengajuan PSP BMN Satker ' . Auth::user()->satker->name,
                'link' => route('Uapb::psp.show', $pengelolaan->id),
            ]);

            $temp->delete();

            return redirect()->route('Satker::psp.index')->withSuccess('Pengajuan PSP berhasil dikirim');
        } catch (\Exception$e) {
    ...
    public function result()
    {
        ...
        return view('satker.psp.result', compact(['temp']));
    }
    ...
    public function draftPerbaikan(Request $request, $id)
    {
        ...
        if (!$item || $item->satker_id != $satker->id) {
            return redirect()->route('Satker::psp.index')
                ->withWarning('Data tidak ditemukan');
        }

        return view('satker.psp.perbaikan', compact(['item']));
    }
    ...
    public function updatePerbaikan(Request $request, $id)
    {
        ...
        try {
            ...
            $this->storeNotif([
                'role' => roles('uapb'),
                'dari' => roles('satker'),
                'content' => 'Permohonan pengajuan perbaikan PSP BMN Satker ' . Auth::user()->satker->name,
                'link' => route('Uapb::psp.show', $item->id),
            ]);

            return redirect()->route('Satker::psp.index')->withSuccess('Perbaikan PSP BMN berhasil dikirim');
        } catch (\Exception$e) {
            return redirect()->back()->withError($e->getMessage())->withInput();
        }
    }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        try {
            ...
            //delete pengelolaan
            $item->delete();
            return redirect()->route('Satker::psp.index')->withSuccess('Hapus Pengelolaan berhasil');
        } catch (\Exception$e) {
            return redirect()->back()->withError($e->getMessage())->withInput();
        }
    }
