...
class HibahAgreementController extends Controller
{
    ...
    public function approve(Request $request, $id)
    {
        ...
        try {
            ...
            $this->storeNotif([
                ...
                'content' => 'Permohonan pengajuan Hibah BMN Satker '.$item->satker->name.' Telah disetujui oleh UAPB',
                'link' => route('Satker::hibah.show', $item->id),
            ]);

            return redirect()->route('Uapb::hibah.show', $item)->withSuccess('Permohonan Hibah Approved');
        } catch (\Exception$e) {
    ...
    public function rejected(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('Uapb::hibah.show', $item)->withSuccess('Pengajuan Rejected');
        } catch (\Exception$e) {
            return redirect()->back()->withError($e->getMessage())->withInput();
        }
    }
    ...
    public function repeat(Request $request, $id)
    {
        ...
        try {
            ...
            $this->storeNotif([
                ...
                'dari' => roles('uapb'),
                'satker_id' => $item->satker->id,
                'content' => 'Permohonan pengajuan Hibah BMN Satker '.$item->satker->name.' perlu perbaikan',
                'link' => route('Satker::hibah.show', $item->id),
            ]);

            return redirect()->route('Uapb::hibah.index', $item)->withSuccess('Pengajuan perlu diperbaiki');
        } catch (\Exception$e) {
            return redirect()->back()->withError($e->getMessage())->withInput();
        }
    }
    ...
    public function upload(Request $request, $id)
    {
        ...
        try {
            ...
            $this->storeNotif([
                ...
                'dari' => roles('uapb'),
                'satker_id' => $item->satker->id,
                'content' => 'Permohonan pengajuan Hibah BMN Satker '.$item->satker->name.' Telah disahkan oleh UAPB',
                'link' => route('Satker::hibah.show', $item->id),
            ]);

            return redirect()->route('Uapb::hibah.show', $item)->withSuccess('Pengesahan Permohoanan HIBAH telah berhasil');
        } catch (\Exception$e) {
            return redirect()->back()->withError($e->getMessage())->withInput();
        }
    }
