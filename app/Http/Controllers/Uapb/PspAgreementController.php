...
class PspAgreementController extends Controller
{
    ...
    public function approve(Request $request, $id)
    {
        ...
        try {
            ...
            $this->storeNotif([
                ...
                'content' => 'Permohonan pengajuan PSP BMN Satker '.$item->satker->name.' Telah disetujui oleh UAPB',
                'link' => route('Satker::psp.show', $item->id),
            ]);

            return redirect()->route('Uapb::psp.show', $item)->withSuccess('PSP approved');
        } catch (\Exception$e) {
    ...
    public function rejected(Request $request, $id)
    {
        ...
        try {
            ...
            $this->storeNotif([
                ...
                'dari' => roles('uapb'),
                'satker_id' => $item->satker->id,
                'content' => 'Permohonan pengajuan PSP BMN Satker '.$item->satker->name.' Tidak Disetujui oleh UAPB',
                'link' => route('Satker::psp.show', $item->id),
            ]);

            return redirect()->route('Uapb::psp.show', $item)->withSuccess('Pengajuan Rejected');
        } catch (\Exception$e) {
            return redirect()->back()->withError($e->getMessage())->withInput();
        }
    }
    ...
    public function repeat(Request $request, $id)
    {
        ...
        try {
            ...
            $this->storeNotif([
                ...
                'dari' => roles('uapb'),
                'satker_id' => $item->satker->id,
                'content' => 'Permohonan pengajuan PSP BMN Satker '.$item->satker->name.' perlu perbaikan',
                'link' => route('Satker::psp.show', $item->id),
            ]);

            return redirect()->route('Uapb::psp.index', $item)->withSuccess('Pengajuan perlu diperbaiki');
        } catch (\Exception$e) {
            return redirect()->back()->withError($e->getMessage())->withInput();
        }
    }
    ...
    public function upload(Request $request, $id)
    {
        ...
        try {
            ...
            $this->storeNotif([
                ...
                'dari' => roles('uapb'),
                'satker_id' => $item->satker->id,
                'content' => 'Permohonan pengajuan PSP BMN Satker '.$item->satker->name.' Telah disahkan oleh UAPB',
                'link' => route('Satker::psp.show', $item->id),
            ]);

            return redirect()->route('Uapb::psp.show', $item)->withSuccess('Pengesahan Pengajuan PSP telah berhasil');
        } catch (\Exception$e) {
            return redirect()->back()->withError($e->getMessage())->withInput();
        }
    }
