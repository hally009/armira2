<div class="card">
    ...
    <div class="card-body wizard-content">
        <div class="form tab-wizard wizard-circle">
            ...
            <h6>Isi Data</h6>
            <section>
                <form id="formData" action="{{ route('Satker::pengelolaan-temp.store', 'hibah') }}" method="POST" enctype="multipart/form-data">
                    @method('POST')
                    @csrf
                    ...
                    <div class="row mb-3 mt-3">
                        ...
                        <div class="col-md-6">
                            <div class="form-floating">
                                <input type="number" class="form-control" id="nilai_hibah" value="{{ ($temp)?$temp->form_view['nilai_hibah']:'' }}" name="nilai_hibah">
                                <label for="nilai_hibah">Total Nilai Objek HIBAH</label>
                            </div>
                        </div>
                    </div>
                </form>
                ...
                <div class="row mb-5">
                    ...
                    <div class="tab-content pt-2" id="borderedTabContent">
                        ...
                        <div class="tab-pane fade @if($loop->iteration == 1)  show active @endif" id="{{ $tab['id'] }}" role="tabpanel">
                            <form id="{{ $tab['id_upload'] }}" action="{{ route('Satker::pengelolaan-temp.upload-file', ['hibah', $tab['name_file']]) }}" method="POST" enctype="multipart/form-data">
                                @method('POST')
                                @csrf
                                <div class="row mb-3 mt-3">
                                    <div class="col-sm-6">
                                        <input type="file" class="form-control" id="file_hibah" name="file_hibah">
                                    </div>
                                    <div class="col-sm-6">
            ...
            <section>
                <form id="submit-form" action="{{ route('Satker::hibah.store') }}" method="POST" enctype="multipart/form-data">
                    @method('POST')
                    @csrf
                </form>
                <div id="sec-result">
                    @include('satker.hibah.result')
                </div>
            </section>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(".tab-wizard").steps({
        ...
        , startIndex: {{($temp) ? $temp->step: 0}} // set default step
